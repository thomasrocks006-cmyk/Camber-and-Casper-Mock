...
import { 
  Bell, CheckCircle2, ChevronDown, Command, LayoutDashboard, MonitorPlay, 
  Phone, Radio, Search, Settings, ShieldAlert, Users, Zap 
} from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const { autonomyMode, setAutonomyMode, viewMode, setViewMode, callConsole } = useAppStore();

  if (location === '/login' || location === '/mode-select') return <>{children}</>;

  const navigation = [
    { path: '/today', label: 'Today', icon: MonitorPlay },
    { path: '/command-center', label: 'Command', icon: LayoutDashboard },
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-foreground dark">
      ...
      <div className={`${viewMode === 'Simple' ? 'w-16' : 'w-64'} transition-[width] duration-200 flex-shrink-0 border-r border-border bg-sidebar flex flex-col z-10`}>
        <div className="h-16 flex items-center px-4 border-b border-border">
          <div className="font-bold text-lg text-sidebar-foreground flex items-center gap-3">
            <img src={logoUrl} alt="Logo" className="w-8 h-8 rounded bg-primary/20 object-cover" />
            {viewMode !== 'Simple' && (
              <div className="flex flex-col">
                <span className="leading-none tracking-tight">Ironbark</span>
                <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">Camber & Casper</span>
              </div>
            )}
          </div>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="w-full flex items-center justify-between px-3 py-2 rounded-md bg-secondary/50 text-sm font-medium hover:bg-secondary transition-colors">
                <span className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${autonomyMode === 'Autonomous Within Policy' ? 'bg-primary' : 'bg-amber-500'}`} />
                  {autonomyMode}
                </span>
            ...
            <DropdownMenuContent className="w-56">
              ...
              {(['Manual Only', 'Review Each Lane', 'Execute Pre-Approved', 'Autonomous Within Policy'] as const).map((mode) => (
                <DropdownMenuItem key={mode} onClick={() => setAutonomyMode(mode)} className="flex items-center justify-between">
                  {mode}
                  {autonomyMode === mode && <CheckCircle2 className="w-4 h-4 text-primary" />}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        <nav className={`flex-1 overflow-y-auto py-4 flex flex-col gap-1 ${viewMode === 'Simple' ? 'px-2 items-center' : 'px-2'}`}>
          {navigation.map(item => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 ${isActive ? 'text-primary' : 'text-sidebar-foreground/50'}`} />
          ...
          <div className="flex items-center gap-4 flex-1">
            <button className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-secondary/50 text-sm text-muted-foreground hover:bg-secondary transition-colors w-64 border border-border/50">
              ...
              <div className="ml-auto flex items-center gap-1">
                <kbd className="px-1.5 py-0.5 rounded bg-background border border-border text-[10px] font-medium font-mono text-muted-foreground">⌘</kbd>
                <kbd className="px-1.5 py-0.5 rounded bg-background border border-border text-[10px] font-medium font-mono text-muted-foreground">K</kbd>
              </div>
            </button>
          </div>
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => setViewMode(viewMode === 'Simple' ? 'Detailed' : 'Simple')}>
                  Switch to {viewMode === 'Simple' ? 'Detailed' : 'Simple'} View
                </DropdownMenuItem>
                <DropdownMenuItem>Profile Settings</DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild><Link href="/login" className="w-full cursor-pointer">Sign out</Link></DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </header>
